'''
1. Write a program to add two lists index-wise. Create a new list that contains
the 0th index item from both the list, then the 1st index item, and so on till
the last element. any leftover items will get added at the end of the new list.  DON'T USE ZIP
list1 = ["M", "na", "i", "Ke"]
list2 = ["y", "me", "s", "lly"]
output = ['My', 'name', 'is', 'Kelly']

2. Given a two Python list. Write a program to iterate both lists simultaneously
and display items from list1 in original order and items from list2 in reverse order. DONT USE ZIP
list1 = [10, 20, 30, 40]
list2 = [100, 200, 300, 400]
output = 
10 400
20 300
30 200
40 100

3. Concatenate two lists in the following order DONT USE ZIP
list1 = ["Hello ", "take "]
list2 = ["Dear", "Sir"]
output = ['Hello Dear', 'Hello Sir', 'take Dear', 'take Sir']
'''